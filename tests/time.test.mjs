...
function packet(unixMs = 1790000000000) {
  ...
  writeTimestamp(request, 40, unixMs);
  const reply = Buffer.alloc(48);
  reply[0] = 0x24;
  reply[1] = 2;
  request.copy(reply, 24, 40, 48);
  writeTimestamp(reply, 32, unixMs + 10);
  writeTimestamp(reply, 40, unixMs + 12);
...
test('NTP timestamp eras work before and after the 2036 wraparound', () => {
  for (const date of ['2026-09-16T01:00:00.125Z', '2037-09-16T01:00:00.125Z']) {
    ...
    writeTimestamp(buf, 0, value);
    assert.ok(Math.abs(readTimestamp(buf, 0, value) - value) < 0.001);
  }
});
test('NTP reply applies delay compensation and rejects invalid origin, mode and leap states', () => {
  ...
  assert.ok(Math.abs(decodeReply(reply, request, 22, unixMs).unixMs - (unixMs + 22)) < 0.001);
  for (const alter of [
    (r) => (r[24] ^= 1),
    (r) => (r[0] = 0xe4),
    (r) => (r[0] = 0x23),
    ...
    const bad = Buffer.from(reply);
    alter(bad);
    assert.throws(() => decodeReply(bad, request, 22, unixMs));
  }
...
test('real UDP exchange closes its socket and reads the response', async () => {
  ...
  try {
    const sample = await queryNtp('127.0.0.1', { port: server.address().port });
    assert.ok(Math.abs(sample.unixMs - Date.now()) < 100);
  } finally {
...
test('NTP timeout rejects and does not leave an open socket', async () => {
  ...
  try {
    await assert.rejects(
      queryNtp('127.0.0.1', { port: server.address().port, timeoutMs: 20 }),
      /timeout/,
    );
  } finally {
    await new Promise((resolve) => server.close(resolve));
...
test('time service deduplicates requests, uses fresh projected cache, and falls back to second provider', async () => {
  let now = 0,
    calls = [];
  const get = createTimeService({
    monotonic: () => now,
    query: async (host) => {
      calls.push(host);
      if (host === 'time.cloudflare.com') throw new Error('offline');
      return { unixMs: 1790000000000, at: now, source: host, uncertaintyMs: 5, stratum: 2 };
    },
  });
  const [a, b] = await Promise.all([get(), get()]);
  assert.equal(a.unixMs, b.unixMs);
  assert.equal(calls.length, 2);
  now = 12000;
  const c = await get();
  assert.equal(c.unixMs, a.unixMs + 12000);
  assert.equal(c.sampleAgeMs, 12000);
  assert.equal(calls.length, 2);
  now = 61000;
  await get();
...
test('browser time chooses minimum RTT, ignores device clock jumps, and expires', async () => {
    ...
    fetcher: async () => {
      ...
      return {
        ...
        json: async () => ({
          ...
          unixMs: 1790000000000 + mono - rtt / 2,
          sampleAgeMs: 0,
          uncertaintyMs: 1,
        }),
      };
  ...
  assert.equal(clock.anchor.rtt, 20);
  assert.equal(clock.synchronized, true);
  const initial = clock.now();
  wall += 864000000;
  mono += 1000;
  assert.equal(clock.now(), initial + 1000);
  mono += 900000;
...
test('failed and malformed API responses cannot silently use the device clock as NTP', async () => {
  ...
  assert.equal(await unavailable.sync(), false);
  assert.equal(unavailable.synchronized, false);
  assert.equal(unavailable.now(), 1234);
  const malformed = new NetworkClock({
    fetcher: async () => ({ ok: true, json: async () => ({ protocol: 'NTP', unixMs: 'bad' }) }),
  });
...
test('time endpoint rejects POST and disables all response caches', async () => {
  const headers = {};
  let body = '';
  const response = { setHeader: (k, v) => (headers[k] = v), end: (text) => (body = text) };
