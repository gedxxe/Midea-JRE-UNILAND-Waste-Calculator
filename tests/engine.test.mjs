...
function value(result, name) {
  return result.calculatedRows.find((r) => r.name === name).totalEnergy;
}
function raw(draft, side = 'start') {
  return (
    `${side === 'start' ? draft.startDate : draft.endDate}\n` +
    PLANT_SCHEMAS[draft.plantKey].rows
      .map(
        (row, i) =>
          `${i + 1}. ${row.name}: ${draft.rows[i][side].map((n, mi) => `${n}${row.ratioLabel?.includes('/') ? ` ((Ratio ${row.ratioLabel.split('/')[0]})/1000)` : row.factors[mi] === 1 ? '' : ` (Ratio ${row.factors[mi]})`}`).join(' + ')}`,
      )
      .join('\n')
  );
}
...
test('UNILAND ratios, units, precision and worksheet sums match the examples', () => {
  const d = complete('UNILAND');
  set(d, 'Total', '712.33', '716.11');
  set(d, 'Building A', '3691.4', '3695.72');
  set(d, 'Indoor', '24095.65', '24228.1');
  set(d, 'Vacum box indoor', '100', '107.7');
  set(d, 'Outdoor', '1000', '1149.1');
  set(d, 'Vacum box outdoor', '2000', '2405.1');
  set(d, 'Line compressor outdoor', '200', '231.05');
  set(d, 'Refrigant and LPG area', '20.08', '22.31');
  set(d, 'Trafo 1', '10', '10.125');
  const r = calculateDraft(d);
  assert.equal(value(r, 'Total'), 12.096);
  assert.equal(value(r, 'Building A'), 0.6912);
  assert.equal(value(r, 'Trafo 1'), 0.125);
  assert.equal(value(r, 'Refrigant and LPG area'), 89.2);
...
test('both reports and JRE worksheet use the first reading date', () => {
  ...
  const uni = calculateDraft(complete('UNILAND'));
  assert.equal(jre.reportDate, '2026-09-16');
  assert.equal(uni.reportDate, '2026-09-16');
...
test('missing meters never become zero or a misleading partial total', () => {
  const d = complete();
  set(d, 'Server Room', ['1', '-'], ['2', '3']);
  const r = calculateDraft(d);
  assert.equal(value(r, 'Server Room'), null);
  assert.equal(r.subAreasSumKWh, null);
  assert.equal(r.gapKWh, null);
  assert.match(r.mainText, /27\. Server Room: -\n/);
  assert.ok(r.issues.some((i) => i.code === 'UNAVAILABLE'));
  set(d, 'Server Room', '1', '2');
...
test('JRE inactive exception needs confirmation and only applies to the two compressor rows', () => {
  const d = complete();
  const ri = set(d, 'Air Compressor 1#', '-', '-');
  assert.equal(value(calculateDraft(d), 'Air Compressor 1#'), null);
  d.rows[ri].inactive = true;
  assert.equal(value(calculateDraft(d), 'Air Compressor 1#'), 0);
  set(d, 'Air Compressor 1#', '-', '400');
  assert.equal(value(calculateDraft(d), 'Air Compressor 1#'), 0);
  const other = set(d, 'Crusher Machine', '-', '-');
  d.rows[other].inactive = true;
...
test('UNILAND missing Trafo and missing grouped worksheet components stay unavailable', () => {
  const d = complete('UNILAND');
  set(d, 'Trafo 1', '-', '-');
  set(d, 'Vacum box indoor', '-', '1');
  const r = calculateDraft(d);
  assert.equal(value(r, 'Trafo 1'), null);
  assert.equal(r.worksheet.derived.indoorArea, null);
...
test('decreasing meters are flagged and excluded, with valid equipment still calculated', () => {
  ...
  set(d, 'Dehumidifier', '2345', '345');
  const r = calculateDraft(d);
  assert.equal(value(r, 'Dehumidifier'), null);
  assert.equal(value(r, 'Total'), 200);
  assert.match(r.reportSectionText, /reading decreased from 2345 to 345/);
  assert.equal(r.gapKWh, null);
});
test('large jumps produce a warning without modifying readings', () => {
  const d = complete();
  set(d, 'Dehumidifier', '1000', '10000');
  const r = calculateDraft(d);
  assert.equal(value(r, 'Dehumidifier'), 9000);
  assert.ok(r.issues.some((i) => i.code === 'SUSPICIOUS_JUMP'));
});
test('blank, invalid, ambiguous and negative raw values block copying', () => {
  for (const value of [
    ...
    const d = complete();
    set(d, 'Total', value, '1234');
    assert.equal(calculateDraft(d).success, false, value);
  }
  assert.equal(parseFlexibleNumber('394,850'), 394.85);
  assert.equal(scaledReading('1000000000000'), 1000000000000000000n);
});
test('decimal subtraction preserves small changes in large counters', () => {
  const d = complete();
  set(d, 'All Office Building', '999999999.123456', '999999999.123457');
  assert.equal(value(calculateDraft(d), 'All Office Building'), 0.001);
  const u = complete('UNILAND');
  set(u, 'Building A', '100.000001', '100.000002');
...
test('invalid or reversed dates cannot be copied', () => {
  for (const [start, end] of [
    ...
    assert.equal(calculateDraft(complete('JRE', start, end)).success, false);
...
test('utilities keep order, units, decimal normalization and water notes', () => {
  const d = complete();
  d.utilities[0].value = '2,50';
  d.utilities[4] = {
    value: '99,26',
    note: 'the process of filling the reservoir tank for hydrant water reserves',
  };
  const r = calculateDraft(d);
  assert.match(r.mainText, /- LPG: 2.5 Kg/);
  assert.match(r.mainText, /- Water: 99.26 m³ \(the process/);
  assert.doesNotMatch(r.mainText, /- Oxygen/);
  d.utilities[2].note = 'note without value';
  assert.equal(calculateDraft(d).success, false);
});
test('importer accepts decimal commas, metadata and wrapped injection readings', () => {
  ...
  const r = parseReading(text, 'JRE');
  assert.equal(r.success, true, JSON.stringify(r.issues));
  assert.equal(r.date, '2026-09-16');
  assert.equal(r.values[6].length, 13);
});
test('importer validates ratios and preserves their warning in calculated reports', () => {
  const d = complete();
  const parsed = parseReading(raw(d).replace('1000 (Ratio 1000)', '1000 (Ratio 1)'), 'JRE');
  assert.equal(parsed.success, true);
  assert.ok(parsed.issues.some((i) => i.code === 'RATIO_MISMATCH'));
...
test('importer blocks two days, duplicate equipment, missing meter and junk suffixes', () => {
  ...
  for (const changed of [
    text + '\n' + text,
    text + '\n1. Total: 20',
    text.replace('27. Server Room: 1000 + 1000', '27. Server Room: 1000'),
    text.replace('1. Total: 1000', '1. Total: 1000 junk'),
    text.replace('1000 + 1000', '1000 + + 1000'),
  ])
    assert.equal(parseReading(changed, 'JRE').success, false);
});
test('TSV paste preserves matrix position and rejects entire oversized or malformed blocks', () => {
  ...
  assert.deepEqual(
    planTablePaste('10\t20\n30\t40\n', coords, 1, 'start').map((c) => c.value),
    ['10', '20', '30', '40'],
  );
...
test('saved drafts restore with exact readings and reject corrupt schema', () => {
  ...
  const saved = JSON.stringify({ version: 4, drafts });
  const restored = restoreDrafts(saved);
  assert.deepEqual(restored.JRE.rows, drafts.JRE.rows);
  drafts.JRE.rows[26].end = ['20'];
  assert.throws(() => restoreDrafts(JSON.stringify({ version: 4, drafts })));
...
test('next day copies only the ending baseline and resets utilities and inactive confirmation', () => {
  const d = complete();
  d.utilities[0].value = '12';
  d.rows[11].inactive = true;
  const next = nextDayDraft(d);
  assert.equal(next.startDate, '2026-09-17');
  assert.equal(next.endDate, '2026-09-18');
  assert.deepEqual(next.rows[0].start, d.rows[0].end);
  assert.equal(next.rows[0].end[0], '');
  assert.equal(next.rows[11].inactive, false);
  assert.equal(next.utilities[0].value, '');
