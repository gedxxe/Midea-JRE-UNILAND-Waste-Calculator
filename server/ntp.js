...
export function decodeReply(reply, request, elapsedMs, referenceMs = Date.now()) {
  if (reply.length < 48 || !reply.subarray(24, 32).equals(request.subarray(40, 48)))
    throw new Error('NTP response does not match the request.');
  const leap = reply[0] >> 6;
  const version = (reply[0] >> 3) & 7;
  const mode = reply[0] & 7;
  if (leap === 3 || ![3, 4].includes(version) || mode !== 4 || reply[1] < 1 || reply[1] > 15)
    throw new Error('NTP server is not synchronized.');
  if (reply.subarray(32, 40).every((b) => b === 0) || reply.subarray(40, 48).every((b) => b === 0))
    throw new Error('NTP timestamps missing.');
  ...
  if (
    processing < -1 ||
    processing > elapsedMs + 2 ||
    elapsedMs < 0 ||
    elapsedMs > 2000 ||
    dispersionMs > 10000
  )
    throw new Error('NTP sample quality is insufficient.');
  const networkDelay = Math.max(0, elapsedMs - processing);
  return {
    unixMs: transmitted + networkDelay / 2,
    uncertaintyMs: networkDelay / 2 + rootDelayMs / 2 + dispersionMs + 1,
    stratum: reply[1],
  };
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error, value) => {
      ...
      try {
        ...
        /* socket may not have bound after a DNS failure */
      }
    ...
    socket.once('message', (message) => {
      ...
      try {
        finish(null, { ...decodeReply(message, request, at - sentAt), at, source: host });
      } catch (error) {
        finish(error);
      }
    });
...
export function createTimeService({
  ...
  return async function getTime() {
      ...
      pending = (async () => {
        ...
        const emit = (event) => {
          try {
            ...
            /* Logging must not break time synchronization. */
          }
        };
        for (const host of ['time.cloudflare.com', 'time.google.com']) {
          try {
            attempts++;
            cached = await query(host);
            emit({ status: 'ok', source: host, attempts, durationMs: monotonic() - started });
            return;
          } catch {
            /* try the second fixed source */
          }
        }
        failedAt = monotonic();
        emit({ status: 'unavailable', attempts, durationMs: failedAt - started });
    ...
    return {
      unixMs: cached.unixMs + age,
      source: cached.source,
      protocol: 'NTP',
      sampleAgeMs: age,
      uncertaintyMs: cached.uncertaintyMs + age * 0.00005,
      stratum: cached.stratum,
    };
  };
}
