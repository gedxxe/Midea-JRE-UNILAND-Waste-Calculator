...
export function buildWorksheet(plantKey, reportDate, rows) {
  ...
  const outdoorArea = outdoor !== null && vacOut !== null && lineComp !== null ? round(outdoor + vacOut + lineComp, 8) : null;

  const headers = ['Indoor Area', 'Outdoor Area', 'Indoor', 'Outdoor', 'HE & Piping', 'Vacuum Box Indoor', 'Vacuum Box Outdoor', 'Outdoor Line Compressor Area', 'Machine Stamping', 'Injection Molding', 'PCB Trial Line', '10HP Enthalpy Lab', '20HP Climate Chamber', 'OQC Testing Room B', 'IQC Testing Room B', 'Charger Forklift Area'];
  const values = [
    indoorArea,
    outdoorArea,
    indoor,
