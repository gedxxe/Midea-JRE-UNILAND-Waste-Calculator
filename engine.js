...
import { decimalText, scaledReading, round, formatNumber, dayDiff, formatReportDate } from './numbers.js';
import { buildWorksheet, worksheetPreview } from './worksheet.js';

export function createDraft(plantKey, startDate = '', endDate = '') {
  return {
    plantKey, startDate, endDate,
...
export function calculateDraft(draft) {
  ...
  if (!schema) throw new Error('Pabrik tidak dikenal.');
  const issues = [...(draft.importIssues || [])];
  const days = dayDiff(draft.startDate, draft.endDate);
  if (days === null) issue(issues, 'DATE_INVALID', 'Isi tanggal awal dan akhir yang valid.', null, null, 'ERROR');
  else if (days <= 0) issue(issues, 'DATE_ORDER', 'Tanggal akhir harus setelah tanggal awal.', null, null, 'ERROR');
  else if (days !== 1) issue(issues, 'DATE_GAP', `Periode ${days} hari (${draft.startDate} sampai ${draft.endDate}); bukan 24 jam.`);
  ...
  const calculatedRows = schema.rows.map((row, ri) => {
    ...
    let missing = false;
    let energyScaled = 0n;
    if (!input || input.start?.length !== row.factors.length || input.end?.length !== row.factors.length) {
      issue(issues, 'METER_COUNT', `${row.name}: harus ada ${row.factors.length} meter pada kedua reading.`, ri, null, 'ERROR');
    ...
    row.factors.forEach((factor, mi) => {
      ...
      if (!start || !end) {
        missing = true;
        issue(issues, 'EMPTY_READING', `${label}: reading ${!start && !end ? 'awal dan akhir' : !start ? 'awal' : 'akhir'} belum diisi.`, ri, mi, 'ERROR');
        return;
      }
      // Only the two JRE exceptions may use an operator-confirmed inactive state.
      if (row.allowInactive && input.inactive && (start === '-' || end === '-') &&
      ...
      if (start === '-' || end === '-') {
        missing = true;
        issue(issues, 'UNAVAILABLE', `${label}: data tidak tersedia (${start} ke ${end}).`, ri, mi);
        return;
      }
      ...
      if (a === null || b === null) {
        missing = true;
        issue(issues, 'INVALID_READING', `${label}: gunakan angka positif, tanpa pemisah ribuan, maksimal 6 desimal dan nilai 1000000000000.`, ri, mi, 'ERROR');
        return;
      }
      const delta = b - a;
      if (delta < 0n) {
        missing = true;
        issue(issues, 'READING_DECREASE', `${label}: reading turun dari ${start} menjadi ${end}. Periksa angka, reset, atau rollover meter.`, ri, mi);
        return;
      }
      const diff = Number(delta) / 1e6;
      if (diff > Math.max(100, Number(a) / 1e6 * 0.5)) {
        issue(issues, 'SUSPICIOUS_JUMP', `${label}: reading berubah dari ${start} menjadi ${end}. Kenaikan melebihi 100 dan 50% reading awal; periksa raw data.`, ri, mi);
      }
  ...
  UTILITIES[draft.plantKey].forEach(([name, unit], i) => {
    ...
    if (!String(value).trim()) {
      if (String(note).trim()) issue(issues, 'UTILITY_VALUE', `${name}: catatan sudah diisi tetapi nilainya belum ada.`, null, null, 'ERROR');
      return;
    }
    const normalized = decimalText(value);
    if (value !== '-' && normalized === null) {
      issue(issues, 'UTILITY_INVALID', `${name}: nilai utility tidak valid.`, null, null, 'ERROR');
      return;
    }
    const cleanNote = String(note).trim().replace(/[\r\n\t]+/g, ' ');
    utilities.push(`${schema.utilityPrefix} ${name}: ${value === '-' ? '-' : formatNumber(Number(normalized))}${unit ? ` ${unit}` : ''}${cleanNote ? ` (${cleanNote})` : ''}`);
  });
  ...
  const subAreasSumKWh = draft.plantKey === 'JRE' && completeSubmeters
    ? round(subRows.reduce((sum, row) => sum + row.totalEnergy, 0), 8) : null;
  const gapKWh = subAreasSumKWh !== null && totalDirectEnergy !== null ? round(totalDirectEnergy - subAreasSumKWh, 8) : null;
  const gapPercent = gapKWh !== null && totalDirectEnergy !== 0 ? gapKWh / totalDirectEnergy * 100 : null;
  ...
  const lines = [`${schema.reportPrefix}) ${schema.name} (${formatReportDate(reportDate)} - ${duration})`];
  for (const row of calculatedRows) {
    lines.push(`${row.no}. ${row.displayName || row.name}: ${formatRowValue(draft.plantKey, row)}${row.missing ? '' : ` ${row.unit}`}`);
  }
  lines.push(...utilities);
  const mainText = lines.join('\n');
  const checks = issues.length ? `CHECK RAW DATA:\n${issues.map(item => `- ${item.message}`).join('\n')}` : '';
  const crossCheckText = draft.plantKey === 'JRE' ? [
    'CROSS-CHECK:',
    `Sub-meter 2-29: ${subAreasSumKWh === null ? 'belum lengkap' : `${subAreasSumKWh.toFixed(2)} kWh`}`,
  ...
  return {
    plantKey: draft.plantKey, reportDate, startDate: draft.startDate, endDate: draft.endDate,
    success: !issues.some(item => item.level === 'ERROR'), issues, calculatedRows, mainText,
    reportSectionText: [mainText, crossCheckText, checks].filter(Boolean).join('\n\n'),
    checks, crossCheckText, worksheet,
    worksheetText: `${draft.plantKey} ELECTRICAL WORKSHEET:\n${worksheetPreview(worksheet)}`,
    totalDirectEnergy, totalDirectUnit: schema.rows[0].unit,
    subAreasSumKWh, gapKWh, gapPercent,
    completeRows: calculatedRows.filter(row => !row.missing).length
  };
}

export function generateFullIndonesiaReport(jre, uniland) {
  if (!jre?.success || !uniland?.success) throw new Error('Lengkapi reading dan tanggal kedua pabrik terlebih dahulu.');
  if (jre.startDate !== uniland.startDate || jre.endDate !== uniland.endDate) {
    throw new Error('Periode reading JRE dan UNILAND harus sama.');
  }
  ...
  return '[INDONESIA FACTORY ENERGY REPORT]\n\nSummary situation:\n' + sections.filter(Boolean).join('\n\n');
}
